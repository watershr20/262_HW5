...
test_that("wildfire_likelihood_checks_errors", {
  ...
  expect_error(wildfire_likelihood(climate_fire_probability = 1.01,
                                   land_fire_probability = 0.20))
  ...
  expect_error(wildfire_likelihood(climate_fire_probability = 0.20,
                                   land_fire_probability = 1.01))
...
#Check that higher climate-based probability -> higher overall probability
  expect_true(wildfire_likelihood(climate_fire_probability = 0.21, 
                                  land_fire_probability = 0.10) >
                wildfire_likelihood(climate_fire_probability = 0.20, 
                                    land_fire_probability = 0.10))
  
#Check that higher land-based probability -> higher overall probability
  expect_true(wildfire_likelihood(climate_fire_probability = 0.10, 
                                  land_fire_probability = 0.21) >
                wildfire_likelihood(climate_fire_probability = 0.10, 
                                    land_fire_probability = 0.20))
  
#Check that maximizing both inputs doesn't give an output greater than 100%
  expect_true(wildfire_likelihood(climate_fire_probability = 1, 
                                  land_fire_probability = 1) < 1)
})